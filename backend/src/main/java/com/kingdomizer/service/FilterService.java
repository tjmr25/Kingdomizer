...
/**
 * Service responsible for filtering Dominion resources based on various
 * criteria.
 * This service handles the filtering logic for both kingdom cards and landscape
 * cards,
...
public class FilterService {
    ...
    /**
     * Generates a random set of 10 kingdom cards based on the provided filter
     * criteria.
     ...
     * @return List of 10 selected kingdom card IDs, sorted by expansion, cost, then
     *         name
     */
    public List<Long> generateKingdomCards(KingdomFilter filter) {
        logger.info("Generating kingdom cards with filter criteria");
        ...
        Collections.shuffle(availableKingdomCards);
     ...
     *
     * @param filter The filter object containing all filtering criteria
     * @return Complete list of potential landscape card resources matching the
     *         filter criteria
     */
    public List<Long> generateLandscapeCards(KingdomFilter filter) {
        logger.info("Generating landscape cards, count requested: {}", filter.getLandscapeCount());
        List<Resource> availableLandscapeCards = getPotentialResources(filter).stream()
                .filter(resource -> Boolean.TRUE.equals(resource.getHasLandscapeOrientation()) &&
                        !ResourceCategory.GAMEPART.equals(resource.getResourceCategory()))
                .collect(Collectors.toList());

        Collections.shuffle(availableLandscapeCards);

        // Check if there are enough landscape cards available
        if (filter.getLandscapeCount() > 0 && availableLandscapeCards.size() < filter.getLandscapeCount()) {
            logger.warn("Not enough landscape cards available. Requested: {}, Found: {}", 
                filter.getLandscapeCount(), availableLandscapeCards.size());
            throw new ImpossibleKingdomException("Not enough landscape cards available. Requested " + 
                filter.getLandscapeCount() + " but only found " + availableLandscapeCards.size());
        }
    ...
    /**
     * Gets all potential resources (both kingdom cards and landscape cards) based
     * on the provided filter criteria.
     * This is the main method that fetches all resources at once to reduce database
     * calls.
     *
     * @param filter The filter object containing all filtering criteria
    ...
    private List<Resource> getPotentialResources(KingdomFilter filter) {
        if (filter.getExpansions().isEmpty()) {
            logger.warn("Kingdom generation failed: No expansions selected");
            throw new ImpossibleKingdomException("No expansions selected");
        } else {
            List<Expansion> selectedExpansions = extractSelectedExpansions(filter.getExpansions());
            logger.debug("Selected expansions: {}", selectedExpansions);
    ...
    private List<Resource> filterUnwantedResources(List<Resource> resources, KingdomFilter filter) {
        ...
        final Set<Long> TOKEN_IDS = Set.of(20019L, 31009L, 21011L); // 20019L: Punktemarker, 31009L: Embargomarker,
                                                                    // 21011L: Geldmarker
        final Set<Long> MAT_IDS = Set.of(30019L, 30020L, 21010L, 31010L); // 30019L: Island, 30020L: Native Village,
                                                                          // 21010L Trade Route, 31010L Pirate Ship
        ...
        return resources.stream()
                .filter(resource -> {
                    ...
                    for (String exclusion : activeExclusions) {
                        switch (exclusion) {
                            case "curses":
                                if ((resource.getCardProperties() != null
                                        && Boolean.TRUE.equals(resource.getCardProperties().getHasCurse()))
                                        || (resource.getCardTypes() != null
                                                && resource.getCardTypes().contains(CardType.CURSE))) {
                            ...
                            case "tokens":
                                if ((resource.getDependencies() != null
                                        && resource.getDependencies().stream().anyMatch(TOKEN_IDS::contains))
                            ...
                            case "tableaus":
                                if ((resource.getDependencies() != null
                                        && resource.getDependencies().stream().anyMatch(MAT_IDS::contains))
    ...
    private List<Resource> filterMandatoryResources(List<Resource> availableCards, KingdomFilter filter) {
        ...
        if (filter.getCardFeatures() != null) {
            ...
            if (features.isHasMultipleActions()) {
                ...
                if (card == null) {
                    logger.warn("Failed to find a card with multiple actions");
                    throw new ImpossibleKingdomException("No card with multiple actions found");
                }
            ...
            if (features.isHasMultipleCards()) {
                ...
                if (card == null) {
                    logger.warn("Failed to find a card with multiple draws");
                    throw new ImpossibleKingdomException("No card with multiple draws found");
                }
                selectedCards.add(card);
            }
            ...
            if (features.isHasBuy()) {
                ...
                if (card == null) {
                    logger.warn("Failed to find a card with +1 Buy");
                    throw new ImpossibleKingdomException("No card with +1 Buy found");
                }
                selectedCards.add(card);
            }
            ...
            if (features.isHasReaction()) {
                ...
                if (card == null) {
                    logger.warn("Failed to find a Reaction card");
                    throw new ImpossibleKingdomException("No Reaction card found");
                }
                selectedCards.add(card);
            }
            ...
            if (features.isHasTrash()) {
                ...
                if (card == null) {
                    logger.warn("Failed to find a card with Trash");
                    throw new ImpossibleKingdomException("No card with Trash found");
                }
                selectedCards.add(card);
            }
        }
        ...
        if (filter.getMinActionCards() != null && filter.getMinActionCards() > 0) {
            ...
            for (int i = 0; i < neededActions; i++) {
                ...
                if (card == null) {
                    logger.warn("Failed to find sufficient +1 Action cards");
                    throw new ImpossibleKingdomException("Not enough +1 Action cards available, can't fulfill requirements");
                }
                selectedCards.add(card);
                actionCount++;
            }
        ...
        if (filter.getMinAttackCards() != null && filter.getMinAttackCards() > 0) {
            ...
            for (int i = 0; i < neededAttacks; i++) {
                ...
                if (card == null) {
                    logger.warn("Failed to find sufficient Attack cards");
                    throw new ImpossibleKingdomException("Not enough Attack cards available, can't fulfill requirements");
                }
                selectedCards.add(card);
                attackCount++;
            }
        ...
        if (selectedCards.size() != 10) {
            logger.warn("Failed to generate kingdom: could not select 10 cards (found {})", selectedCards.size());
            throw new ImpossibleKingdomException("Kingdom could not be generated with the given filter criteria");
        }

        logger.info("Successfully selected 10 kingdom cards");
     ...
     * @return The found card (removed from the original list), or null if none
     *         found
     */
